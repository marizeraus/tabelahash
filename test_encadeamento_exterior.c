...
if (!skip) {
	...
	int end = busca(50, NOME_ARQUIVO_HASH, NOME_ARQUIVO_DADOS, M);
...
if (!skip) {
	...
	int end = insere(50, "Mariana", NOME_ARQUIVO_HASH, NOME_ARQUIVO_DADOS, 1, M);
	ASSERT_EQUAL(end, 1);
...
if (!skip) {
	...
	int end = insere(49, "Jorge", NOME_ARQUIVO_HASH, NOME_ARQUIVO_DADOS, 1, M);
	ASSERT_EQUAL(end, -1);
...
if (!skip) {
	...
	int end = insere(10, "Ana", NOME_ARQUIVO_HASH, NOME_ARQUIVO_DADOS, 6, M);
	ASSERT_EQUAL(end, 6);
...
if (!skip) {
	...
	int end = exclui(10, NOME_ARQUIVO_HASH, NOME_ARQUIVO_DADOS, M);
	ASSERT_EQUAL(end, -1);
...
if (!skip) {
	...
	int end = exclui(59, NOME_ARQUIVO_HASH, NOME_ARQUIVO_DADOS, M);
	ASSERT_EQUAL(end, 1);

	tab_hash_saida = le_compartimentos(NOME_ARQUIVO_HASH);
	ASSERT_EQUAL_CMP(tab_hash, tab_hash_saida, cmp_compartimentos);

	tab_cliente->lista[1]->ocupado = LIBERADO;
	tab_cliente_saida = le_clientes(NOME_ARQUIVO_DADOS);
	ASSERT_EQUAL_CMP(tab_cliente, tab_cliente_saida, cmp_clientes);
...
if (!skip) {
	...
	int end = exclui(87, NOME_ARQUIVO_HASH, NOME_ARQUIVO_DADOS, M);
	ASSERT_EQUAL(end, 5);

	tab_hash_saida = le_compartimentos(NOME_ARQUIVO_HASH);
	ASSERT_EQUAL_CMP(tab_hash, tab_hash_saida, cmp_compartimentos);

	tab_cliente->lista[5]->ocupado = LIBERADO;
	tab_cliente_saida = le_clientes(NOME_ARQUIVO_DADOS);
	ASSERT_EQUAL_CMP(tab_cliente, tab_cliente_saida, cmp_clientes);
